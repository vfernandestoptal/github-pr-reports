...
function calculateStatistics(name, reviews, config) {
    ...
    const timeBuckets = helpers.range(config.maxBuckets).map((_, index) => {
        return {
            minValue: index * config.bucketSize,
            maxValue:
                index + 1 === config.maxBuckets
                    ? ''
                    : (index + 1) * config.bucketSize,
            count: 0,
        };
    });
    ...
    reviewTimes.forEach(time => {
        const bucketIndex = Math.min(
            Math.floor(moment.duration(time).asHours() / config.bucketSize),
            config.maxBuckets - 1
        );
...
function generate(data, options) {
    ...
    async.map(
        ...
        (user, callback) => {
            ...
            async.setImmediate(() =>
                callback(null, calculateStatistics(user, reviews, config))
            );
        },
        (err, users) => {
            if (err) {
